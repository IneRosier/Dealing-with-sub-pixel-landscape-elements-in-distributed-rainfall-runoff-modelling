...
outlet_id = 734
scaling = "FlLength_manning"# "FlLength_Ks" , "none", "gamma", "std"
Ke_range = [0.72] #[19.2, 1.92, 0.96, 0.48, 0.192]
Ke_vLE = 102.4
manning_vLE = 0.43
...
vLE = ['FA305', 'FA400', 'FA500', 'FA600', 'FA700', 'FA800', 'no_vLE'] 
weight = "Full" # "FA" or "" or "Full"
    ...
    for Ke_value in Ke_range:
        for vLE_type in vLE:
            ...
            if rainfall_event == "20060821":
                ...
                storm_duration = 17460 #single storm event
            ...
            if rainfall_event == "20070809":
                ...
                storm_duration = 76920 #single storm event
            ...
            if rainfall_event == "20070821":
                ...
                elapsed_time = 0.0
                model_run_time = 100000
                storm_duration = 70200 #single storm event
            ...
            if weight == "" or weight == "Full":
                if vLE_type == "FA305":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_FA305.tif')
                ...
                if vLE_type == "FA400":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_FA400.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
                if vLE_type == "FA500":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_FA500.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)                
                if vLE_type == "FA600":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_FA600.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
                if vLE_type == "FA700":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_FA700.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)                
                if vLE_type == "FA800":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_FA800.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
                if vLE_type == "no_vLE":
                    vLE_config = np.zeros(1152)                                         
            if weight == "FA":
                if vLE_type == "FA305":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_WeightFA305.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
                if vLE_type == "FA400":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_WeightFA400.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
                if vLE_type == "FA500":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_WeightFA500.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)                
                if vLE_type == "FA600":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_WeightFA600.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
                if vLE_type == "FA700":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_WeightFA700.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)                
                if vLE_type == "FA800":
                    vLE_config = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\Rasters_vLE_landscape\\subc1_20m_WeightFA800.tif')
                    vLE_config = vLE_config.read()
                    vLE_config = np.flip(vLE_config, 1)
                    vLE_config = vLE_config.flatten()
                    vLE_config = vLE_config.astype(np.float64)
            ...
            main = "D:\\1_phd\\WP3\\Landlab_output\\subcatchment1\\"
            output_folder = os.path.join(main,
                                         "20m_subc1_vLEType_" + vLE_type + "_Ke" +
                                         str(Ke_value) +
                                         "scaling_" + scaling +
                                         "_weight_" + weight +
            ...
            if DTM_slope =="velm":
                if DTM_type == "heulengracht_burn":
                    watershed_DEM = "C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\subc1_20m_dtm_halo.asc"
            (rmg, z) = read_esri_ascii(watershed_DEM, name="topographic__elevation")
            rmg.set_watershed_boundary_condition_outlet_id(734, z, nodata_value=-9999.0) # outlet = node 734
            ...
            if year == 2006:
                manning = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\landcover\\manning_2006_20m.tif')
            if year == 2007:
                manning = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\landcover\\manning_2007_20m.tif')
            if year == 2013:
                manning = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\landcover\\manning_2013_20m.tif')
            if year == 2016:
                manning = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\landcover\\manning_2016_20m.tif')            
            if year == 2019:
                manning = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\landcover\\manning_2019_vLE_20m.tif')             
                print(manning)
            config = manning
            manning = manning.read()
            manning = np.flip(manning, 1)
    
            fllength_scaling = rasterio.open('C:\\Users\\u0117123\OneDrive - KU Leuven\\FWO\\WP3\\Subcatchment1\\FlLengthScaling\\flowlength_scaling_20m.tif')
            fllength_scaling = fllength_scaling.read()
            fllength_scaling[fllength_scaling<0] = 1
            fllength_scaling = np.flip(fllength_scaling, 1)
            fllength_scaling = fllength_scaling.flatten()
            ...
            manning_rmg_vLE =  (vLE_config_inv * manning) + (vLE_config*(manning_vLE))
            if scaling == "FlLength_manning":
                manning_rmg_vLE = manning_rmg_vLE * fllength_scaling
            rmg.at_node['mannings_n__coefficients'] = manning_rmg_vLE
            ### saturated hydraulic conductivity 
            rows, columns = config.shape
            tau_gamma = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\WP3\\Subcatchment1\\fang_et_al\\gamma_con_20m.tif')
            tau_gamma = tau_gamma.read()
            tau_gamma[tau_gamma<0] = 1
            tau_gamma = np.flip(tau_gamma, 1)
            tau_std = rasterio.open('C:\\Users\\u0117123\\OneDrive - KU Leuven\\FWO\WP3\\Subcatchment1\\fang_et_al\\stdev_con_20m2.tif')
            ...
            dataframe_c734= pd.DataFrame(columns=['time', 'timestep', 'depth', 'depth_before_inf', 
                                                  'depth_after_inf', 'infiltration_depth', 'actual_infiltration',
                                                  'h_beforeInf_734', 'h_afterInf_734', 'discharge_734', 'surface_water_discharge_734', 'rainfall_intensity'])
               
            inf_t0 = sum(rmg.at_node['soil_water_infiltration__depth'])
            inf_t0_c734 = (rmg.at_node['soil_water_infiltration__depth'])[734]
            hydrograph_time = []
            discharge_at_outlet = []
            ...
            while elapsed_time < model_run_time:
                ...
                # #define rain intensity for time step   
                # if elapsed_time < (storm_duration):
                #     if rainfall_intensity_df['time'].iloc[a+1]>elapsed_time:
                #         if rainfall_intensity_df['time'].iloc[a+1]>=elapsed_time + timestep:
                #             rainfall_mmhr = rainfall_intensity_df['P_int'].iloc[a]
                ...
                #     rainfall_mmhr = 0.0
                #     of.rainfall_intensity = 0.0
                print('run ' + str(run) + 'elapsed_time ' + str(elapsed_time) + "- rainfall intensity: " + str(rainfall_mmhr))
                #define rain intensity for time step   
                # if elapsed_time < (storm_duration):
                #     if rainfall_intensity_df['time'].iloc[a+1]>elapsed_time:
                ...
                discharge = of._q
                discharge_734 = rmg.calc_flux_div_at_node(of._q)[734]
                ...
                surface_water_discharge_734 = rmg.at_node['surface_water__discharge'][734]
                # model infiltration
                h_beforeInf_sum = sum(rmg.at_node['surface_water__depth'])
                h_beforeInf_734 = rmg.at_node['surface_water__depth'][734]
                #timestep = of.dt
                SI.run_one_step(timestep)
                h_afterInf_sum = sum(rmg.at_node['surface_water__depth'])
                h_afterInf_734 = rmg.at_node['surface_water__depth'][734]
                ...
                dataframe_c734 = dataframe_c734.append(pd.DataFrame({'time': elapsed_time,
                                                                     'timestep': of.dt,
                                                                     'depth': (rmg.at_node['surface_water__depth'])[734],
                                                                     'depth_before_inf': depth_before_inf[734], 
                                                                     'depth_after_inf': depth_after_inf[734],
                                                                     'infiltration_depth':(rmg.at_node['soil_water_infiltration__depth'])[734],
                                                                     'actual_infiltration':(rmg.at_node['soil_water_infiltration__depth'])[734] - inf_t0_c734,
                                                                     'h_beforeInf_734': h_beforeInf_734,
                                                                     'h_afterInf_734': h_afterInf_734,
                                                                     'discharge_734': discharge_734,
                                                                     'surface_water_discharge_734':surface_water_discharge_734,
                                                                     'rainfall_intensity': of.rainfall_intensity}, 
                                                                    index=[0]), ignore_index=True)
                #check water balance
                (unique, counts) = np.unique(rmg.status_at_node, return_counts=True)
                #water out
                dataframe_int['Q_volume'] = dataframe_int['timestep'] * dataframe_c734['surface_water_discharge_734']
                ...
                #water in
                dataframe_int['P_volume'] = dataframe_int['timestep'] * dataframe_c734['rainfall_intensity'] * counts[0] * (rmg.dx**2)
                ...
                discharge_at_outlet.append(np.abs(of._q[734]) * rmg.dx) # append discharge in m^3/s
            ...
            list_dfs = [dataframe_int, dataframe_c734]
            list_dfs_names = ["int","c734"]
            ...
            #discharge graph
            ax = dataframe_c734.plot(x='time', y='surface_water_discharge_734', style='.', ms=1)
            ax.set_ylabel("discharge (m³/sec)")
            ax.get_figure().savefig(output_folder + '\\discharge.png')
            
            #Output file
            #discharge volume (m³) sum
            dataframe_c734['discharge_volume'] = dataframe_c734['timestep'] * dataframe_c734['surface_water_discharge_734']
            sum_discharge_734  = dataframe_c734['discharge_volume'].sum()
            
            #peak discharge (m³/sec)
        
            peak_discharge734 = dataframe_c734['surface_water_discharge_734'].max()
                                
            #time of peak discharge
            peak_discharge_df734 = dataframe_c734[dataframe_c734['surface_water_discharge_734']==dataframe_c734['surface_water_discharge_734'].max()]
            peak_discharge_time734 = peak_discharge_df734['time'].iloc[0]
            #infiltration sum (m³)
            ...
            output_df = output_df.append(pd.DataFrame({
                ...
                'peak_discharge': peak_discharge734, 
                'peak_discharge_time': peak_discharge_time734, 
...
# manning_rmg_vLE =  (vLE_config_inv * manning) + (vLE_config*(manning_vLE))
# #%%                    
# folder = "C:\\Users\\u0117123\\Desktop\\delete2\\"
# numpy_array_to_raster(folder + "manning_rmg_vLE" + '.tif',
#                       manning_rmg_vLE,
#                       bottom_left,
#                       cell_resolution,
#                       nband = 1,
